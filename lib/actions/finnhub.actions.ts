...
async function fetchJSON<T>(
  ...
  if (!res.ok) {
    ...
    try {
      const maskedUrl = url.replace(/(token=)[^&]*/i, "$1[REDACTED]");
      console.error(`Fetch failed ${res.status} for ${maskedUrl}: ${text}`);
    } catch (e) {
      console.error(`Fetch failed ${res.status} and failed to mask url: ${text}`);
    }
